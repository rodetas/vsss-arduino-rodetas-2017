...
double last_output_1 = 0;
double second_last_error_1 = 0;

double last_error_2 = 0;
double last_output_2 = 0;
double second_last_error_2 = 0;
...
int PID_1(double setpoint, double input) {
  ...
  double proportional = error - last_error_1;
  double integral = error * dt;
  double derivative = (error - (2 * last_error_1) - second_last_error_1) / dt;

  double output = last_output_1 + KP * proportional + KI * integral + KD * derivative;

  if (output > 255) output = 255;
  if (output < 0) output = 0;
  
  last_error_1 = error;
  second_last_error_1 = last_error_1;
  last_output_1 = output;
...
int PID_2(double setpoint, double input) {
  ...
  double error = setpoint - input;
  double proportional = error - last_error_2;
  double integral = error * dt;
  double derivative = (error - (2 * last_error_2) - second_last_error_2) / dt;

  double output = last_output_2 + KP * proportional + KI * integral + KD * derivative;

  if (output > 255) output = 255;
  if (output < 0) output = 0;
  
  last_error_2 = error;
  second_last_error_2 = last_error_2;
  last_output_2 = output;
